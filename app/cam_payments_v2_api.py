...
router = APIRouter(prefix="/api/cam", tags=["cam-payments"])
...
def _update_invoice_status(conn, invoice_id: int) -> None:
    row = conn.execute(
        "SELECT total_cents,amount_paid_cents,credit_applied_cents,status FROM academy_invoices WHERE id=?",
        (invoice_id,),
    ).fetchone()
    if not row:
    ...
    paid = int(row["amount_paid_cents"]) + int(row["credit_applied_cents"])
    status = "paid" if balance == 0 else "partially_paid" if paid > 0 else "open"
    conn.execute(
        "UPDATE academy_invoices SET status=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",
        (status, invoice_id),
    )
...
def _payment(payment_id: int) -> dict:
    ...
    row["allocations"] = fetch_all(
        """
        SELECT pa.*,i.invoice_number,i.total_cents,i.amount_paid_cents,i.credit_applied_cents,
...
def post_payment(payload: PaymentPayload):
    ...
    allocation_pairs = [(item.invoice_id, item.amount_cents) for item in payload.allocations]
    ...
    with connection() as conn:
        for invoice_id, amount in allocation_pairs:
            invoice = conn.execute(
                "SELECT account_id,total_cents,amount_paid_cents,credit_applied_cents,status FROM academy_invoices WHERE id=?",
                (invoice_id,),
            ).fetchone()
            if not invoice:
            ...
            if amount > _invoice_balance(invoice):
        ...
        row = conn.execute(
            ...
            (
                ...
                payload.method, payload.received_on, payload.idempotency_key,
                _clean(payload.external_reference), _clean(payload.notes),
            ),
        ).fetchone()
        payment_id = int(row["id"])
        conn.execute("UPDATE academy_payments SET receipt_number=? WHERE id=?", (f"RCT-{payment_id:06d}", payment_id))
...
def refund_payment(payment_id: int, payload: RefundPayload):
    ...
    existing = fetch_one(
        "SELECT id,payment_id,amount_cents,refunded_on,reason FROM academy_refunds WHERE idempotency_key=?",
        (payload.idempotency_key,),
    )
    if existing:
        if (
            int(existing["payment_id"]) != payment_id
            or int(existing["amount_cents"]) != payload.amount_cents
            or str(existing["refunded_on"]) != payload.refunded_on
        ):
    ...
    with connection() as conn:
        payment_row = conn.execute(
            "SELECT amount_cents,unapplied_cents,refunded_cents FROM academy_payments WHERE id=?",
            (payment_id,),
        ).fetchone()
        if not payment_row:
            raise HTTPException(404, "Payment not found")
        ...
        # Reverse unused family credit first because it has no invoice allocation.
        credit_refund = min(amount_left, int(payment_row["unapplied_cents"]))
        if credit_refund:
            conn.execute(
                """
                UPDATE academy_payments
                SET unapplied_cents=unapplied_cents-?,refunded_cents=refunded_cents+?,updated_at=CURRENT_TIMESTAMP
                WHERE id=?
                """,
        ...
        # Then reverse invoice allocations newest-first while preserving original rows.
        if amount_left:
            ...
            for allocation in allocations:
                ...
                # Invariant: an allocation reversal can never exceed the currently posted amount from that allocation.
                conn.execute(
                    "UPDATE academy_invoices SET amount_paid_cents=amount_paid_cents-?,updated_at=CURRENT_TIMESTAMP WHERE id=?",
                    (reversal, int(allocation["invoice_id"])),
                )
                conn.execute(
                    "UPDATE academy_payments SET allocated_cents=allocated_cents-?,refunded_cents=refunded_cents+?,updated_at=CURRENT_TIMESTAMP WHERE id=?",
    ...
    return {"refund": fetch_one("SELECT * FROM academy_refunds WHERE id=?", (refund_id,)), "payment": _payment(payment_id)}


@router.get("/payments/{payment_id}/receipt")
def payment_receipt(payment_id: int):
    payment_row = _payment(payment_id)
    account = _account(int(payment_row["account_id"]))
    ...
    return {
        ...
        "account_id": payment_row["account_id"],
        "account_name": payment_row["account_name"],
        "amount_cents": payment_row["amount_cents"],
        "method": payment_row["method"],
        "received_on": payment_row["received_on"],
        "external_reference": payment_row["external_reference"],
        "status": payment_row["status"],
        "allocations": allocations,
        "unapplied_credit_cents": payment_row["unapplied_cents"],
        "refunded_cents": payment_row["refunded_cents"],
...
def billing_account_ledger(account_id: int):
    ...
    invoices = fetch_all(
        ...
        SELECT id,invoice_number,issue_date,due_date,status,total_cents,amount_paid_cents,credit_applied_cents,
               CASE
                   WHEN total_cents-amount_paid_cents-credit_applied_cents < 0 THEN 0
                   ELSE total_cents-amount_paid_cents-credit_applied_cents
