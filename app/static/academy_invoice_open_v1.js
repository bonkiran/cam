(() => {
  ...
  function tab(){
    ...
    if(page!=='cam')return null;return new URLSearchParams(query).get('tab')||'overview';
  }
  ...
  function detailHtml(invoice){
    ...
    return `<article class="panel cam-invoice-detail"><div class="cam-form-title"><div><span class="cam-kicker">INVOICE DETAIL</span><h2>${esc(invoice.invoice_number||`Invoice ${invoice.id}`)}</h2><p>${esc(invoice.account_name||'Family account')} · Issued ${esc(invoice.issue_date)} · Due ${esc(invoice.due_date)}</p></div><button type="button" class="secondary" data-close-invoice-detail>Close</button></div>
      <div class="cam-invoice-detail-status"><span class="cam-program-status ${esc(invoice.status)}">${esc(invoice.status)}</span><strong>Balance ${money(invoice.balance_due_cents)}</strong></div>
      <div class="cam-invoice-detail-items">${items.length?items.map(item=>`<div><div><strong>${esc(item.description)}</strong><small>Qty ${Number(item.quantity||1)} · ${money(item.unit_amount_cents)} each</small></div><span>${money(item.line_total_cents)}</span></div>`).join(''):'<div><span>No invoice items found.</span></div>'}</div>
      <div class="cam-invoice-detail-totals"><div><span>Subtotal</span><strong>${money(invoice.subtotal_cents)}</strong></div><div><span>Discount</span><strong>${money(invoice.discount_cents)}</strong></div><div><span>Invoice total</span><strong>${money(invoice.total_cents)}</strong></div><div><span>Paid</span><strong>${money(invoice.amount_paid_cents)}</strong></div><div><span>Credit applied</span><strong>${money(invoice.credit_applied_cents)}</strong></div><div class="balance"><span>Balance due</span><strong>${money(invoice.balance_due_cents)}</strong></div></div>
    </article>`;
  }
  async function openInvoice(id,button){
    const old=button.textContent;button.disabled=true;button.textContent='Opening…';
    try{
      const invoice=await json(`/api/cam/invoices/${id}`);
  ...
  async function enhance(){
    ...
    const rows=$$('.cam-invoice-row');if(!rows.length)return;
    const pending=rows.filter(row=>row.dataset.invoiceOpenEnhanced!=='1');if(!pending.length)return;
    enhancing=true;
    try{
      const invoices=await json('/api/cam/invoices');
      ...
      pending.forEach(row=>{
        ...
        const status=$(':scope > .cam-program-status',row);
        const actions=document.createElement('div');actions.className='cam-invoice-actions';
  ...
  window.addEventListener('hashchange',schedule);window.addEventListener('cam-payments-updated',schedule);document.addEventListener('DOMContentLoaded',schedule);
