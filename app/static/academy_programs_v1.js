(() => {
  ...
  function ensureTab(){
    const tabs=$('.academy-tabs'); if(!tabs)return;
    let btn=$('#academyProgramsTab',tabs);
    if(!btn){
      btn=document.createElement('button');
      btn.id='academyProgramsTab';
      btn.textContent='Programs & Enrollment';
      btn.onclick=()=>{location.hash='academy?tab=programs';};
  ...
  function select(label,name,value,options,required=false){
    return `<label class="academy-field"><span>${label}${required?' *':''}</span><select name="${name}" ${required?'required':''}><option value="">Select</option>${options.map(o=>{const val=typeof o==='string'?o:o.value;const text=typeof o==='string'?o:o.label;return `<option value="${esc(val)}" ${String(value||'')===String(val)?'selected':''}>${esc(text)}</option>`;}).join('')}</select></label>`;
  }
  ...
  function programForm(p={}){
    const editing=!!p.id;
    return `<form id="academyProgramForm" class="panel academy-form-card" data-program-id="${esc(p.id||'')}">
      <div class="academy-form-title"><div><span class="academy-kicker">SLICE 2A · ${editing?'EDIT PROGRAM':'NEW PROGRAM'}</span><h2>${editing?'Update Program':'Create Program'}</h2><p>Define a reusable academy training program. Billing linkage is intentionally deferred until Fees & Payments.</p></div><button type="button" class="secondary" id="cancelProgramForm">Cancel</button></div>
      <div class="academy-form-section"><div><h2>Program Definition</h2><p>Name, target group and lifecycle.</p></div><div class="academy-form-grid three">
      ...
      <div class="academy-form-actions"><span id="programSaveStatus"></span><button type="submit" class="primary">${editing?'Save Program':'Create Program'}</button></div>
    </form>`;
  }
  function enrollmentForm(players,programs){
    return `<form id="academyEnrollmentForm" class="panel academy-form-card">
      <div class="academy-form-title"><div><span class="academy-kicker">SLICE 2A · NEW ENROLLMENT</span><h2>Enroll Player</h2><p>Create a regular or trial enrollment. Only active players and active programs are eligible.</p></div><button type="button" class="secondary" id="cancelEnrollmentForm">Cancel</button></div>
      <div class="academy-form-section"><div><h2>Enrollment</h2><p>Program membership and effective dates.</p></div><div class="academy-form-grid three">
      ...
      <div class="academy-form-actions"><span id="enrollmentSaveStatus"></span><button type="submit" class="primary">Create Enrollment</button></div>
    </form>`;
  }
  function actionForm(enrollment,action){
    const freeze=action==='freeze';
    return `<form id="academyEnrollmentActionForm" class="panel academy-form-card" data-enrollment-id="${enrollment.id}" data-action="${action}">
      <div class="academy-form-title"><div><span class="academy-kicker">ENROLLMENT ACTION</span><h2>${freeze?'Freeze':'Cancel'} Enrollment</h2><p>${esc(enrollment.player_name)} · ${esc(enrollment.program_name)}</p></div><button type="button" class="secondary" id="cancelEnrollmentAction">Cancel</button></div>
      <div class="academy-form-section"><div><h2>${freeze?'Freeze details':'Cancellation details'}</h2><p>This changes lifecycle status without deleting history.</p></div><div class="academy-form-grid two">
      ...
      <div class="academy-form-actions"><span id="enrollmentActionStatus"></span><button type="submit" class="primary">Confirm ${freeze?'Freeze':'Cancellation'}</button></div>
    </form>`;
  }
  function programRows(programs){
    if(!programs.length)return `<div class="academy-program-empty"><strong>No programs yet</strong>Create the first academy program before enrolling players.</div>`;
    return programs.map(p=>`<div class="academy-program-row" data-program-row="${p.id}"><div><strong>${esc(p.name)}</strong><small>${esc(p.description||'No description')}</small><div class="academy-program-tags"><span>${esc(p.program_type||'group')}</span>${p.age_group?`<span>${esc(p.age_group)}</span>`:''}${p.skill_level?`<span>${esc(p.skill_level)}</span>`:''}<span>${Number(p.current_enrollment_count||0)} current</span></div></div><div class="academy-program-actions"><span class="academy-program-status ${esc(p.status)}">${esc(p.status)}</span><button data-edit-program="${p.id}">Edit</button></div></div>`).join('');
  }
  function enrollmentRows(enrollments){
    if(!enrollments.length)return `<div class="academy-program-empty"><strong>No enrollment history yet</strong>Enroll an active player into an active program.</div>`;
    return enrollments.map(e=>{
      const canFreeze=e.status==='active';
      const canCancel=e.status!=='cancelled';
      return `<div class="academy-enrollment-row" data-enrollment-row="${e.id}"><div><strong>${esc(e.player_name)} → ${esc(e.program_name)}</strong><div class="academy-enrollment-meta"><span class="academy-enrollment-type">${esc(e.enrollment_type)}</span><span>Start: ${esc(e.start_date||'—')}</span>${e.end_date?`<span>End: ${esc(e.end_date)}</span>`:''}${e.frozen_on?`<span>Frozen: ${esc(e.frozen_on)}</span>`:''}${e.cancelled_on?`<span>Cancelled: ${esc(e.cancelled_on)}</span>`:''}</div>${e.cancellation_reason?`<small>Reason: ${esc(e.cancellation_reason)}</small>`:''}</div><div class="academy-enrollment-actions"><span class="academy-program-status ${esc(e.status)}">${esc(e.status)}</span>${canFreeze?`<button class="warn" data-freeze-enrollment="${e.id}">Freeze</button>`:''}${canCancel?`<button class="danger" data-cancel-enrollment="${e.id}">Cancel</button>`:''}</div></div>`;
    }).join('');
  }
  function pageHtml(programs,enrollments,players){
    ...
    return `<section class="academy-section-head"><div><span class="academy-kicker">SLICE 2A · PROGRAMS & ENROLLMENT</span><h1>Programs & Enrollment</h1><p>Define academy offerings and manage each player's enrollment lifecycle without creating a second player identity.</p></div><div class="academy-hero-actions"><button class="secondary" id="openEnrollmentForm">＋ Enroll Player</button><button class="primary" id="openProgramForm">＋ Add Program</button></div></section>
      <section class="academy-stats"><article class="academy-stat green"><div class="academy-stat-icon">▦</div><div><span>Active programs</span><strong>${activePrograms.length}</strong><small>${programs.length} total</small></div></article><article class="academy-stat blue"><div class="academy-stat-icon">♙</div><div><span>Current enrollments</span><strong>${active}</strong><small>Active memberships</small></div></article><article class="academy-stat amber"><div class="academy-stat-icon">◷</div><div><span>Active trials</span><strong>${trials}</strong><small>Trial enrollment type</small></div></article><article class="academy-stat gray"><div class="academy-stat-icon">◎</div><div><span>Frozen</span><strong>${frozen}</strong><small>History retained</small></div></article></section>
      <div id="programEditor" class="academy-program-editor"></div><div id="enrollmentEditor" class="academy-program-editor"></div>
      <section class="academy-programs-grid"><article class="panel academy-programs-panel"><div class="panel-head"><div><h2>Programs</h2><p>${programs.length} program${programs.length===1?'':'s'} defined.</p></div></div><div class="academy-program-list">${programRows(programs)}</div><div class="academy-programs-note"><strong>Billing linkage:</strong> Fee plans and enrollment discounts will connect here when the Fees & Payments slice is built; no financial values are fabricated in this slice.</div></article>
      <article class="panel academy-programs-panel"><div class="panel-head"><div><h2>Enrollment History</h2><p>${enrollments.length} enrollment record${enrollments.length===1?'':'s'} retained across active, frozen and cancelled states.</p></div></div><div class="academy-enrollment-list">${enrollmentRows(enrollments)}</div></article></section>`;
  }
  ...
  async function renderPrograms(force=false){
    ...
    content.innerHTML='<div class="panel academy-loading">Loading programs and enrollment history…</div>';
    try{
      const [programs,enrollments,players]=await Promise.all([
        requestJson('/api/academy/programs'),requestJson('/api/academy/enrollments'),requestJson('/api/academy/players')
      ]);
  ...
  function wireProgramForm(editor){
    ...
    $('#academyProgramForm',editor).onsubmit=async e=>{
      ...
      try{await requestJson(id?`/api/academy/programs/${id}`:'/api/academy/programs',{method:id?'PUT':'POST',body:JSON.stringify(payload)});notify(id?'Program updated.':'Program created.');await renderPrograms(true);}catch(err){status.textContent=err.message;submit.disabled=false;}
    };
  }
  function wireEnrollmentForm(editor){
    $('#cancelEnrollmentForm',editor).onclick=()=>editor.innerHTML='';
    $('#academyEnrollmentForm',editor).onsubmit=async e=>{
      ...
      try{await requestJson('/api/academy/enrollments',{method:'POST',body:JSON.stringify(payload)});notify('Enrollment created.');await renderPrograms(true);}catch(err){status.textContent=err.message;submit.disabled=false;}
    };
  }
  function openEnrollmentAction(enrollments,id,action){
    ...
    $('#academyEnrollmentActionForm',editor).onsubmit=async e=>{
      ...
      try{await requestJson(`/api/academy/enrollments/${id}/${action}`,{method:'POST',body:JSON.stringify(payload)});notify(action==='freeze'?'Enrollment frozen.':'Enrollment cancelled.');await renderPrograms(true);}catch(err){status.textContent=err.message;submit.disabled=false;}
    };
  }
  function enhance(){
    scheduled=false;
    if(!$('#academyWorkspace'))return;
    ensureTab();
