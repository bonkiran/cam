(() => {
  ...
  function ensureWorkspace() {
    ...
    if (!main) return null;
    let workspace = $('#camWorkspace', main);
    ...
    workspace.id = 'camWorkspace';
    workspace.innerHTML = '<div class="cam-content" data-core-route=""></div>';
  ...
  function field(label, name, value = '', type = 'text', required = false, placeholder = '') {
    return `<label class="cam-field"><span>${esc(label)}${required?' *':''}</span><input type="${esc(type)}" name="${esc(name)}" value="${esc(value || '')}" ${required?'required':''} placeholder="${esc(placeholder)}"></label>`;
  }

  function selectField(label, name, value, options) {
    return `<label class="cam-field"><span>${esc(label)}</span><select name="${esc(name)}"><option value="">Select</option>${options.map(option => `<option value="${esc(option)}" ${String(value || '') === String(option) ? 'selected' : ''}>${esc(option)}</option>`).join('')}</select></label>`;
  }

  function textArea(label, name, value = '') {
    return `<label class="cam-field cam-field-wide"><span>${esc(label)}</span><textarea name="${esc(name)}" rows="3">${esc(value || '')}</textarea></label>`;
  }

  function setupMarkup(profile = {}) {
    return `<section class="cam-section-head"><div><span class="cam-kicker">ACADEMY PROFILE</span><h1>Settings</h1><p>Maintain the academy identity, contact information, primary location and timezone.</p></div></section>
      <form id="camProfileForm" class="panel cam-form-card">
        <div class="cam-form-section"><div><h2>Academy Information</h2><p>Core organization and contact information.</p></div><div class="cam-form-grid two">
        ...
        <div class="cam-form-section"><div><h2>Primary Location</h2><p>Primary academy address and local timezone.</p></div><div class="cam-form-grid two">
        ...
        <div class="cam-form-actions"><span id="camSaveStatus"></span><button type="submit" class="primary">Save Academy Profile</button></div>
      </form>`;
  }
  ...
  function playerFormMarkup(player = {}) {
    ...
    return `<form id="camPlayerForm" class="panel cam-form-card" data-player-id="${esc(player.id || '')}">
      <div class="cam-form-title"><div><span class="cam-kicker">${editing?'EDIT PLAYER':'NEW PLAYER'}</span><h2>${editing?'Update Player':'Add Player'}</h2><p>Maintain the player and guardian record used throughout academy operations.</p></div><button type="button" class="secondary" id="cancelPlayerForm">Cancel</button></div>
      <div class="cam-form-section"><div><h2>Player Information</h2><p>Identity and academy status.</p></div><div class="cam-form-grid three">
      ...
      <div class="cam-form-section"><div><h2>Cricket Profile</h2><p>Initial cricket characteristics.</p></div><div class="cam-form-grid three">
      ...
      <div class="cam-form-section"><div><h2>Player Contact & Address</h2></div><div class="cam-form-grid three">
      ...
      <div class="cam-form-section"><div><h2>Emergency Contact</h2></div><div class="cam-form-grid two">${field('Emergency Contact Name','emergency_contact_name',player.emergency_contact_name)}${field('Emergency Contact Phone','emergency_contact_phone',player.emergency_contact_phone)}</div></div>
      <div class="cam-form-section"><div class="cam-section-row"><div><h2>Parents / Guardians</h2><p>Add one or more guardian contacts.</p></div><button type="button" class="secondary" id="addGuardian">＋ Add Guardian</button></div><div id="guardianList" class="guardian-list">${guardians.map((guardian, index) => guardianMarkup(guardian, index)).join('')}</div></div>
      <div class="cam-form-section"><div><h2>Internal Notes</h2></div><div class="cam-form-grid">${textArea('Notes','notes',player.notes)}</div></div>
      <div class="cam-form-actions"><span id="playerSaveStatus"></span><button type="submit" class="primary">${editing?'Save Changes':'Create Player'}</button></div>
    </form>`;
  }

  function playerRows(players) {
    if (!players.length) return '<div class="cam-empty"><strong>No academy players yet</strong><span>Add the first player to begin.</span></div>';
    return players.map(player => {
      ...
      return `<div class="cam-player-row detailed" data-player-id="${Number(player.id)}"><div class="cam-avatar">${esc(initials)}</div><div><strong>${esc(player.name)}</strong><small>${esc(secondary)}</small><small>${primary?`Guardian: ${esc(primary.first_name)} ${esc(primary.last_name)}${primary.phone?` · ${esc(primary.phone)}`:''}`:'Guardian not added'}</small></div><span class="cam-status ${esc(player.status || 'active')}">${esc(player.status || 'active')}</span><div class="cam-row-actions"><button data-edit-player="${Number(player.id)}">Edit</button></div></div>`;
    }).join('');
  }

  function playersMarkup(players) {
    return `<section class="cam-section-head"><div><span class="cam-kicker">PLAYER DIRECTORY</span><h1>Players</h1><p>Create and maintain player, cricket, emergency and guardian information.</p></div><button class="primary" id="addAcademyPlayer">＋ Add Player</button></section>
      <div id="playerEditor"></div>
      <article class="panel cam-player-panel"><div class="panel-head"><div><h2>Player Records</h2><p>${players.length} total player${players.length===1?'':'s'}.</p></div></div><div class="cam-player-list">${playerRows(players)}</div></article>`;
  }
  ...
  async function openPlayerEditor(playerId = null) {
    ...
    if (!target) return;
    target.innerHTML = '<div class="panel cam-loading">Loading player…</div>';
    try {
      const player = playerId ? await requestJson(`/api/cam/players/${playerId}`) : {};
      ...
      wireGuardians(target);
      $('#camPlayerForm', target).onsubmit = savePlayer;
    } catch (error) {
  ...
  async function savePlayer(event) {
    ...
    try {
      await requestJson(id ? `/api/cam/players/${id}` : '/api/cam/players', {method:id?'PUT':'POST', body:JSON.stringify(payload)});
  ...
  async function renderSetup(content, token) {
    try {
      const data = await requestJson('/api/cam/profile');
      if (token !== renderToken || route().tab !== 'setup' || !content.isConnected) return;
      content.innerHTML = setupMarkup(data?.profile || data || {});
      const form = $('#camProfileForm', content);
      if (form) form.onsubmit = async event => {
        event.preventDefault();
        const status = $('#camSaveStatus', form);
        const submit = $('button[type="submit"]', form);
        submit.disabled = true;
        if (status) status.textContent = 'Saving…';
        try {
          await requestJson('/api/cam/profile', {method:'PUT', body:JSON.stringify(formObject(form))});
  ...
  async function renderPlayers(content, token) {
    try {
      const players = await requestJson('/api/cam/players');
  ...
  async function renderCoreRoute(force = false) {
    if (!camActive()) return;
    const current = route();
    if (!force && activeTab === current.tab && $('#camWorkspace .cam-content')) return;
    activeTab = current.tab;
    const token = ++renderToken;
    const content = force ? (() => {
      const workspace = ensureWorkspace();
      const old = $('.cam-content', workspace);
      const next = document.createElement('div');
      next.className = 'cam-content';
