(() => {
  ...
  function panelHtml(skills, focus) {
    ...
    const groupHtml = [...groups.entries()].map(([category, items]) => `
      <div class="academy-development-skill-group">
        <strong>${esc(category)}</strong>
        <div class="academy-development-skill-chips">
          ${items.map(skill => `
            <button
              type="button"
              class="academy-development-skill-chip ${selected.has(skill.skill_key) ? 'selected' : ''}"
    ...
    return `
      <section class="academy-development-focus" data-claim-level="training-exposure-only">
        <div class="academy-development-focus-head">
          <div>
            <span class="academy-kicker">MOAT · PLAYER DEVELOPMENT</span>
          ...
          <div class="academy-development-focus-count"><strong data-selected-count>${selected.size}</strong><span>/ ${MAX_SKILLS} selected</span></div>
        </div>
        <div class="academy-development-skill-groups">${groupHtml}</div>
        <div class="academy-development-focus-actions">
          <small><strong>Evidence rule:</strong> this records <em>Practiced / Exposed</em>. It does not claim the player improved.</small>
          <div>
            <span class="academy-development-focus-status" aria-live="polite"></span>
  ...
  async function wireForm(form) {
    ...
    const roster = form.querySelector('.academy-attendance-roster');
    if (!sessionId || !roster) return;

    const loading = document.createElement('section');
    loading.className = 'academy-development-focus academy-development-loading';
    loading.innerHTML = '<strong>Loading development focus…</strong>';
    roster.before(loading);

    try {
      const [skills, focus] = await Promise.all([
        api('/api/academy/development/skills'),
        api(`/api/academy/sessions/${sessionId}/development-focus`)
      ]);
      loading.outerHTML = panelHtml(skills, focus);
      const panel = form.querySelector('.academy-development-focus');
      if (!panel) return;
      ...
      panel.querySelectorAll('[data-development-skill]').forEach(button => {
        button.addEventListener('click', () => {
          ...
          if (!pressed && selectedKeys(panel).length >= MAX_SKILLS) {
            const status = panel.querySelector('.academy-development-focus-status');
      ...
      panel.querySelector('[data-save-development-focus]')?.addEventListener('click', async event => {
        ...
        try {
          const saved = await api(`/api/academy/sessions/${sessionId}/development-focus`, {
            method: 'PUT',
      ...
      loading.classList.add('academy-development-error');
      loading.innerHTML = `<strong>Development focus unavailable</strong><small>${esc(error.message)}</small>`;
    }
  }

  function scan() {
    document.querySelectorAll('#academyAttendanceForm').forEach(form => wireForm(form));
  }
