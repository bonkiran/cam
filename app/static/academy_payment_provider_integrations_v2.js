(() => {
  ...
  function componentMarkup() {
    return `<article class="panel cam-provider-intro">
      <div>
  ...
  async function loadProviders(host) {
    ...
    try {
      ...
      if (host.isConnected) grid.innerHTML = `<div class="warning">Payment providers could not load: ${esc(error.message)}</div>`;
    }
  }
  ...
  async function mount() {
    scheduled = false;
    if (activeRoute() !== ROUTE || mounting || document.querySelector(ROOT_SELECTOR)) return;
    const main = contentHost();
    if (!main) return;

    mounting = true;
    try {
      const host = document.createElement('section');
      host.dataset.camPaymentIntegrations = '1';
      host.className = 'cam-payment-integrations-embedded';
      host.innerHTML = componentMarkup();

      const pageHead = main.querySelector(':scope > .page-head');
      if (pageHead) {
        pageHead.insertAdjacentElement('afterend', host);
      } else {
  ...
  new MutationObserver(() => {
    if (activeRoute() === ROUTE && !document.querySelector(ROOT_SELECTOR)) schedule();
