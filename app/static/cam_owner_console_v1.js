(() => {
  ...
  function go(tab,params={}){
    ...
    location.hash=tab==='overview'?'cam':`academy?${query.toString()}`;
  }
  ...
  function resetOwnerTabs(){
    ...
    $$('button',tabs).forEach(button=>{button.hidden=false;button.classList.remove('cam-owner-legacy-tab');});
  }
  function normalizeOwnerTabs(){
    ...
    $$('button',tabs).forEach(button=>{
      ...
      button.hidden=true;
      button.classList.add('cam-owner-legacy-tab');
    });
    ...
    TOP_TABS.forEach(([tab,label])=>{
      ...
      if(!button){
        ...
        button.dataset.camTab=tab;
        button.className='cam-owner-console-tab';
  ...
  function contextNav(items,active){
    return `<nav class="cam-owner-context-nav" aria-label="Academy workspace sections">${items.map(([id,label,tab,params={}])=>`<button type="button" class="${id===active?'active':''}" data-owner-context-tab="${esc(tab)}" data-owner-context-params="${esc(JSON.stringify(params))}">${esc(label)}</button>`).join('')}</nav>`;
  }
  ...
  function addProgramsContext(){
    ...
    const content=$('#camWorkspace .cam-content');if(!content||$('.cam-owner-context-nav',content))return;
  ...
  function settingsHtml(){
    const cards=[
      ['cam-profile','Academy Profile','Organization, primary location and academy contact information.','setup','⚙'],
    ...
    return `<section class="cam-owner-settings-shell"><section class="cam-section-head"><div><span class="cam-kicker">OWNER / ADMIN</span><h1>Settings</h1><p>Configuration is kept out of daily academy operations and grouped here for authorized administrators.</p></div></section><div class="cam-owner-settings-grid">${cards.map(([id,title,text,target,icon])=>`<button type="button" class="panel cam-owner-settings-card" data-owner-settings-target="${esc(target)}"><span>${icon}</span><div><strong>${esc(title)}</strong><small>${esc(text)}</small></div><b>→</b></button>`).join('')}</div><article class="panel cam-owner-integration-note"><div><h2>Integration architecture remains intact</h2><p>Academy pages call provider adapters. Track B.0 will make each provider configuration cam-specific without changing this navigation model.</p></div><span>SAAS READY SHELL</span></article></section>`;
  }
  function renderSettings(){
    const info=route();if(info.tab!=='settings')return;
    const content=$('#camWorkspace .cam-content');if(!content)return;
    if(!$('.cam-owner-settings-shell',content))content.innerHTML=settingsHtml();
  ...
  function textLine(label,value){return `<div class="cam-owner-detail-line"><span>${esc(label)}</span><strong>${esc(value||'—')}</strong></div>`;}
  function guardianCard(g){
    ...
    return `<div class="cam-owner-person-card"><strong>${esc(`${g.first_name||''} ${g.last_name||''}`.trim()||'Guardian')}</strong><small>${esc(g.relationship||'Parent / Guardian')}${flags?` · ${esc(flags)}`:''}</small><span>${esc(g.phone||'No phone')}</span><span>${esc(g.email||'No email')}</span></div>`;
  }
  function player360Html(data){
    ...
    return `<section class="cam-owner-player360"><section class="cam-section-head cam-owner-player360-head"><div><span class="cam-kicker">PLAYER 360</span><h1>${esc(fullName)}</h1><p>One operational record for player, family, coaching, attendance, academy assignments and connected cricket data.</p></div><div class="cam-owner-head-actions"><button class="secondary" data-owner-player-action="directory">← Player Directory</button><button class="primary" data-owner-player-action="edit">Edit Player</button></div></section><section class="cam-owner-player-summary"><article><span>Status</span><strong>${esc(p.status||'active')}</strong></article><article><span>Joined</span><strong>${esc(p.joined_on||'—')}</strong></article><article><span>Current Batches</span><strong>${batches.length}</strong></article><article><span>Coach Reviews</span><strong>${Number(data.review_count||0)}</strong></article></section><section class="cam-owner-player-grid"><article class="panel"><div class="panel-head"><div><h2>Player Bio</h2><p>Core identity, cricket and emergency information.</p></div></div>${textLine('Preferred name',p.preferred_name)}${textLine('Date of birth',p.date_of_birth)}${textLine('Skill level',p.skill_level)}${textLine('Batting',p.batting_style)}${textLine('Bowling',p.bowling_style)}${textLine('Handedness',p.handedness)}${textLine('Phone',p.phone)}${textLine('Email',p.email)}${textLine('Emergency contact',p.emergency_contact_name)}${textLine('Emergency phone',p.emergency_contact_phone)}</article><article class="panel"><div class="panel-head"><div><h2>Parents & Guardians</h2><p>Family, billing and pickup contacts.</p></div></div><div class="cam-owner-person-list">${guardians.length?guardians.map(guardianCard).join(''):'<div class="cam-dash-empty">No guardian records.</div>'}</div></article><article class="panel"><div class="panel-head"><div><h2>Academy Assignments</h2><p>Current programs, batches and coaching relationships.</p></div></div><div class="cam-owner-assignment-list">${batches.length?batches.map(b=>`<div><strong>${esc(b.batch_name)}</strong><small>${esc(b.program_name||'No program')} · ${esc(b.status||'active')}</small></div>`).join(''):'<div class="cam-dash-empty">No current batch assignment.</div>'}</div><div class="cam-owner-assignment-list">${coaches.length?coaches.map(c=>`<div><strong>${esc(`${c.first_name||''} ${c.last_name||''}`.trim())}</strong><small>${esc(c.assignment_role||'coach')} · since ${esc(c.start_date||'not set')}</small></div>`).join(''):'<div class="cam-dash-empty">No active player coach assignment.</div>'}</div></article><article class="panel"><div class="panel-head"><div><h2>Attendance</h2><p>Recorded player attendance history.</p></div><button class="secondary" data-owner-player-action="attendance">Open Attendance</button></div><div class="cam-owner-attendance-grid"><span><b>${Number(att.present||0)}</b>Present</span><span><b>${Number(att.late||0)}</b>Late</span><span><b>${Number(att.absent||0)}</b>No-show</span><span><b>${Number(att.excused||0)}</b>Excused</span></div></article><article class="panel"><div class="panel-head"><div><h2>CricClubs Profile</h2><p>External player identity stays adapter-based for SaaS compatibility.</p></div></div><div class="cam-owner-integration-state"><strong>${data.cricclubs?.status==='connected'?'Connected':'Not connected'}</strong><span>API access and credentials will be configured per academy in the integration layer.</span></div></article><article class="panel"><div class="panel-head"><div><h2>Reviews, Requests & Complaints</h2><p>Coach development history plus parent/guardian operational feedback.</p></div><button class="secondary" data-owner-player-action="reviews">Coach Reviews</button></div><div class="cam-owner-request-state"><span>Coach reviews</span><strong>${Number(data.review_count||0)}</strong></div><div class="cam-owner-request-state"><span>Parent/Guardian requests & complaints</span><strong>${data.requests_complaints?.status==='not_configured'?'Workflow next':'0'}</strong></div></article></section></section>`;
  }
  async function renderPlayer360(){
    ...
    content.innerHTML='<div class="cam-loading">Loading Player 360…</div>';
    try{
      const data=await api(`/api/cam/owner-console/players/${id}/summary`);
  ...
  async function enhancePlayerDirectory(){
    ...
    const list=$('#camWorkspace .cam-player-list');if(!list||list.dataset.ownerConsole==='1')return;
    list.dataset.ownerConsole='1';
    let directory=[];try{directory=await api('/api/cam/owner-console/players');}catch{}
    const byId=new Map(directory.map(p=>[String(p.id),p]));
    $$('.cam-player-row[data-player-id]',list).forEach(row=>{
      ...
      if(copy&&data){
        ...
        meta.className='cam-owner-player-meta';
      ...
      const actions=$('.cam-row-actions',row);
  ...
  function moneyUnavailable(label){return `<div class="cam-owner-money-card"><span>${esc(label)}</span><strong>—</strong><small>Finance ledger not enabled yet</small></div>`;}
  function dashboardSnapshotHtml(batches,players,asOf){
    ...
    return `<section id="camOwnerSnapshot" class="cam-owner-dashboard-snapshot"><article class="panel"><div class="panel-head"><div><h2>Batch Breakdown</h2><p>Current active player count by batch.</p></div><button class="secondary" data-owner-snapshot-go="batches">Manage</button></div><div class="cam-owner-batch-breakdown">${active.length?active.map(b=>`<div><span>${esc(b.name)}</span><strong>${Number(b.active_player_count||0)}</strong><small>${Number(b.waitlist_count||0)} waitlisted · capacity ${Number(b.capacity||0)}</small></div>`).join(''):'<div class="cam-dash-empty">No active batches.</div>'}</div></article><article class="panel"><div class="panel-head"><div><h2>New Player Registrations</h2><p>${esc(month)} · current month.</p></div><button class="secondary" data-owner-snapshot-go="players">Players</button></div><strong class="cam-owner-registration-count">${registrations.length}</strong><div class="cam-owner-registration-list">${registrations.slice(0,5).map(p=>`<span>${esc(p.name)} <small>${esc(p.joined_on||'')}</small></span>`).join('')||'<span>No new registrations this month.</span>'}</div></article><article class="panel cam-owner-outgoings"><div class="panel-head"><div><h2>Current Month Academy Outgoings</h2><p>Coach salary, facility payments and operating expenses will be recorded in Finance.</p></div><button class="secondary" data-owner-snapshot-go="fees">Finance</button></div><div class="cam-owner-money-grid">${moneyUnavailable('Coach Salary Paid')}${moneyUnavailable('Facility Payments')}${moneyUnavailable('Academy Expenses')}</div></article></section>`;
  }

  async function dashboardData(){
    if(dashboardCache&&Date.now()-dashboardCacheAt<5000)return dashboardCache;
    dashboardCache=await api('/api/cam/dashboard/operations');dashboardCacheAt=Date.now();return dashboardCache;
  }
  async function enhanceDashboardSnapshot(){
    const info=route();if(info.tab!=='overview')return;
    const content=$('#camWorkspace .cam-content');if(!content||$('#camOwnerSnapshot',content))return;
    const stats=$('.cam-stats',content);if(!stats)return;
    try{
      const [batches,players,data]=await Promise.all([api('/api/cam/batches'),api('/api/cam/owner-console/players'),dashboardData()]);
      if(route().tab!=='overview'||$('#camOwnerSnapshot',content))return;
  ...
  async function queueSessionNotification(session,eventType,channels,message,metadata={}){
    ...
    return api('/api/cam/owner-console/notification-events',{method:'POST',body:JSON.stringify({event_type:eventType,entity_type:'session',entity_id:Number(session.id),channels,message,metadata})});
  }
  function refreshDashboard(){
    dashboardCache=null;dashboardCacheAt=0;
    const content=$('#camWorkspace .cam-content');
    if(content){delete content.dataset.dashboardV2;content.innerHTML='<div class="cam-loading">Refreshing dashboard…</div>';}
    window.dispatchEvent(new Event('cam-payments-updated'));
    schedule();
  }
  async function rescheduleSession(sessionId){
    let session;try{session=await api(`/api/cam/sessions/${sessionId}`);}catch(error){notify(error.message);return;}
    const dialog=openDialog(`<form method="dialog" id="camOwnerRescheduleForm"><div class="cam-owner-dialog-head"><div><span>SESSION ACTION</span><h2>Reschedule Session</h2><p>${esc(session.batch_name||'1-to-1 Session')} · ${esc(session.session_date)} ${esc(session.start_time)}</p></div><button type="button" data-owner-dialog-close>×</button></div><div class="cam-form-grid two"><label class="cam-field"><span>New Date *</span><input name="session_date" type="date" value="${esc(session.session_date)}" required></label><label class="cam-field"><span>New Time *</span><input name="start_time" type="time" value="${esc(session.start_time)}" required></label></div>${notificationOptions()}<div class="cam-form-actions"><span id="camOwnerActionStatus"></span><button type="submit" class="primary">Save Reschedule</button></div></form>`);
    const form=$('#camOwnerRescheduleForm',dialog);
    form.onsubmit=async event=>{
      ...
      try{
        ...
        const updated=await api(`/api/cam/sessions/${sessionId}`,{method:'PUT',body:JSON.stringify(payload)});
      ...
      }catch(error){$('#camOwnerActionStatus',form).textContent=error.message;button.disabled=false;}
    };
  }
  async function cancelSession(sessionId){
    let session;try{session=await api(`/api/cam/sessions/${sessionId}`);}catch(error){notify(error.message);return;}
    const dialog=openDialog(`<form method="dialog" id="camOwnerCancelForm"><div class="cam-owner-dialog-head"><div><span>SESSION ACTION</span><h2>Cancel Session</h2><p>${esc(session.batch_name||'1-to-1 Session')} · ${esc(session.session_date)} ${esc(session.start_time)}</p></div><button type="button" data-owner-dialog-close>×</button></div><label class="cam-field"><span>Reason</span><textarea name="reason" rows="3" placeholder="Weather, facility closure, coach unavailable…"></textarea></label>${notificationOptions()}<div class="cam-form-actions"><span id="camOwnerActionStatus"></span><button type="submit" class="danger">Confirm Cancellation</button></div></form>`);
    const form=$('#camOwnerCancelForm',dialog);
    form.onsubmit=async event=>{
      event.preventDefault();const button=form.querySelector('button[type="submit"]');button.disabled=true;
      try{
        const fd=new FormData(form);const reason=String(fd.get('reason')||'').trim();
        const cancelled=await api(`/api/cam/sessions/${sessionId}/cancel`,{method:'POST',body:JSON.stringify({reason:reason||null})});
        const channels=channelsFromForm(form);
  ...
  async function enhanceDashboardActions(){
    const info=route();if(info.tab!=='overview')return;
    const content=$('#camWorkspace .cam-content');if(!content||!$('.cam-dash-session',content))return;
    if($('.cam-owner-session-actions',content))return;
    let data;try{data=await dashboardData();}catch{return;}
    const sessions=[...(data.today_sessions?.group||[]),...(data.today_sessions?.private||[])];
    const rows=$$('.cam-dash-session',content);
    rows.forEach((row,index)=>{
      const session=sessions[index];if(!session)return;
      const actions=document.createElement('div');actions.className='cam-owner-session-actions';
  ...
  async function apply(){
    ...
    applying=true;
    try{
      const info=route();if(info.page!=='cam')return;
