(() => {
  ...
  function ensureAcademyNav(){
    ...
    if(!nav || qs('[data-route="cam"]', nav)) return;
    const players = qs('[data-route="players"]', nav);
    const button = document.createElement('button');
    button.dataset.route = 'cam';
    button.className = camRoute().page === 'cam' ? 'active' : '';
  ...
  function tabBar(active){
    return `<div class="cam-tabs">${tabs.map(([id,label]) =>
      `<button class="${active===id?'active':''}" data-cam-tab="${id}">${label}</button>`
    ).join('')}</div>`;
  }

  function metric(label, value, note, kind='green'){
    return `<article class="cam-stat ${kind}"><div class="cam-stat-icon">${kind==='green'?'♙':kind==='blue'?'▦':kind==='amber'?'◷':'◎'}</div><div><span>${label}</span><strong>${value}</strong><small>${note}</small></div></article>`;
  }

  function moduleCard(icon, title, text, tab, meta='Foundation'){
    return `<button class="cam-module" data-cam-tab="${tab}">
      <span class="cam-module-icon">${icon}</span>
      <span class="cam-module-copy"><strong>${title}</strong><small>${text}</small></span>
      <span class="cam-module-meta">${meta}</span><b>→</b>
    </button>`;
  }

  function emptyOperational(title, description, fields){
    return `<section class="cam-two-col">
      <article class="panel cam-foundation-card">
        <div class="cam-kicker">ACADEMY OPERATIONS · FOUNDATION</div>
        <h2>${title}</h2>
        <p>${description}</p>
        <div class="cam-field-list">${fields.map(x=>`<span>${x}</span>`).join('')}</div>
        <div class="cam-foundation-note"><strong>No fabricated academy data.</strong> This screen is injected now as the product foundation; persistence/workflows will be connected module-by-module.</div>
      </article>
      <article class="panel cam-link-card">
        <div class="cam-kicker">CONNECTED PLAYER DEVELOPMENT</div>
  ...
  function playerRows(players){
    ...
    return players.map(p => `<div class="cam-player-row" data-search="${String(p.name||'').toLowerCase()}">
      <div class="cam-avatar">${String(p.name||'?').split(/\s+/).map(x=>x[0]).join('').slice(0,2).toUpperCase()}</div>
      <div><strong>${p.name}</strong><small>${p.video_count || 0} video(s) · ${p.completed_analyses || 0} completed analysis(es)</small></div>
      <span class="cam-status">Unified profile</span>
  ...
  function overview(data){
    ...
    return `
      <section class="cam-hero">
        <div><span class="cam-kicker">ACADEMY + PERFORMANCE INTELLIGENCE</span><h1>Academy Dashboard</h1><p>Run the academy and develop the cricketer from one longitudinal player record.</p></div>
        <div class="cam-hero-actions"><button class="secondary" data-cam-tab="players">Manage Players</button><button class="primary" data-go-route="upload">＋ Add Player Video</button></div>
      </section>
      <section class="cam-stats">
      ...
      <section class="cam-dashboard-grid">
        <article class="panel cam-operations">
          <div class="panel-head"><div><h2>Academy Operations</h2><p>Management modules inspired by real academy workflows, without separating them from player development.</p></div><span class="cam-badge">PHASE 1</span></div>
        ...
        <aside class="cam-side-stack">
          <article class="panel cam-principle">
            <span class="cam-kicker">PRODUCT PRINCIPLE</span>
            ...
            <div class="cam-flow"><span>Academy</span><b>＋</b><span>Coaching</span><b>＋</b><span>Analysis</span><b>=</b><span>Development proof</span></div>
          </article>
          <article class="panel cam-today">
            ...
            <div class="cam-empty compact"><strong>No sessions configured</strong><span>Next implementation slice: Players → Batches → Coaches → Sessions/Attendance.</span></div>
          </article>
        </aside>
      </section>`;
  }
  ...
  function playersTab(players){
    return `<section class="cam-section-head"><div><span class="cam-kicker">UNIFIED PLAYER DIRECTORY</span><h1>Academy Players</h1><p>Existing CrickAnalysis players are the starting point for academy enrollment and development history.</p></div><button class="primary" data-go-route="upload">＋ Add Player Video</button></section>
      <article class="panel cam-player-panel"><div class="panel-head"><div><h2>Player Records</h2><p>These counts come from the current CrickAnalysis data store.</p></div><span>${players.length}</span></div><div class="cam-player-list">${playerRows(players)}</div></article>`;
  }
  ...
  async function renderAcademy(){
    const {page, tab} = camRoute();
    ensureAcademyNav();
    qsa('.sidebar .nav button').forEach(b=>b.classList.toggle('active', b.dataset.route === 'cam' && page === 'cam'));
    if(page !== 'cam') return;
    ...
    if(camRoute().page !== 'cam') return;

    const topbar = qs('.topbar', main);
    qsa(':scope > *', main).forEach(child=>{ if(child !== topbar) child.remove(); });
    const wrapper = document.createElement('div');
    wrapper.id = 'camWorkspace';
    wrapper.innerHTML = `${tabBar(tab)}<div class="cam-content">${tabContent(tab, data)}</div>`;
    main.appendChild(wrapper);

    qsa('[data-cam-tab]', wrapper).forEach(btn=>btn.onclick=()=>go(btn.dataset.camTab));
