(() => {
  ...
  function ensureNav(){
    ...
    if(!nav) return;
    let btn=qs('[data-route="academy"]',nav);
    if(!btn){
      btn=document.createElement('button');
      btn.dataset.route='academy';
    ...
    qsa('button',nav).forEach(x=>{
      if(x.dataset.route==='academy') x.classList.toggle('active',route().page==='academy');
    });
  }
  ...
  function metric(label,value,note,kind='green'){
    ...
    return `<article class="academy-stat ${kind}"><div class="academy-stat-icon">${icon}</div><div><span>${label}</span><strong>${value}</strong><small>${note}</small></div></article>`;
  }
  ...
  function foundation(title,description,fields){
    return `<section class="academy-two-col"><article class="panel academy-foundation-card"><div class="academy-kicker">ACADEMY OPERATIONS · FOUNDATION</div><h2>${title}</h2><p>${description}</p><div class="academy-field-list">${fields.map(x=>`<span>${x}</span>`).join('')}</div><div class="academy-foundation-note"><strong>No fabricated academy data.</strong> This module is the product foundation; persistence and workflows will be connected module-by-module.</div></article><article class="panel academy-link-card"><div class="academy-kicker">CONNECTED PLAYER DEVELOPMENT</div><h2>Use the same player record</h2><p>Academy operations attach to the existing CrickAnalysis player, video, event and biomechanics history instead of creating another player identity.</p><div class="academy-journey"><span>Enrollment</span><i>→</i><span>Session</span><i>→</i><span>Coach feedback</span><i>→</i><span>Video evidence</span><i>→</i><span>Progress</span></div></article></section>`;
  }
  ...
  function overview(data){
    ...
    return `<section class="academy-hero"><div><span class="academy-kicker">ACADEMY + PERFORMANCE INTELLIGENCE</span><h1>Academy Dashboard</h1><p>Run the academy and develop the cricketer from one longitudinal player record.</p></div><div class="academy-hero-actions"><button class="secondary" data-academy-tab="players">Manage Players</button><button class="primary" data-go-route="upload">＋ Add Player Video</button></div></section>
    <section class="academy-stats">${metric('Players',players.length,'Real CrickAnalysis player profiles','green')}${metric('Players with analysis',withAnalysis,'Have completed development evidence','blue')}${metric('Completed analyses',Number(d.completed_count||0),'Existing video-analysis history','amber')}${metric('Coaches / Batches','Setup','Operational records not configured yet','gray')}</section>
    <section class="academy-dashboard-grid"><article class="panel academy-operations"><div class="panel-head"><div><h2>Academy Operations</h2><p>Management modules connected directly to player development.</p></div><span class="academy-badge">PHASE 1</span></div><div class="academy-module-grid">
    ...
    </div></article><aside class="academy-side-stack"><article class="panel academy-principle"><span class="academy-kicker">PRODUCT PRINCIPLE</span><h2>One player. One history.</h2><p>Attendance, payments, coaching notes, matches, videos and biomechanics attach to the same player profile.</p><div class="academy-flow"><span>Academy</span><b>＋</b><span>Coaching</span><b>＋</b><span>Analysis</span><b>=</b><span>Development proof</span></div></article><article class="panel academy-today"><div class="panel-head"><div><h2>Academy Today</h2><p>Operational scheduling will appear here as sessions are connected.</p></div></div><div class="academy-empty compact"><strong>No sessions configured</strong><span>Next implementation slice: Players → Batches → Coaches → Sessions/Attendance.</span></div></article></aside></section>`;
  }

  function content(tab,data){
    if(tab==='overview') return overview(data);
    if(tab==='players') return `<section class="academy-section-head"><div><span class="academy-kicker">UNIFIED PLAYER DIRECTORY</span><h1>Academy Players</h1><p>Existing CrickAnalysis players become the academy enrollment and development record.</p></div><button class="primary" data-go-route="upload">＋ Add Player Video</button></section><article class="panel academy-player-panel"><div class="panel-head"><div><h2>Player Records</h2><p>These counts come from the current CrickAnalysis data store.</p></div><span>${(data.players||[]).length}</span></div><div class="academy-player-list">${playerRows(data.players||[])}</div></article>`;
  ...
  async function render(){
    ...
    wrap.id='academyWorkspace';
    wrap.innerHTML=`${tabs(current.tab)}<div class="academy-content">${content(current.tab,data)}</div>`;
    main.appendChild(wrap);
    qsa('[data-academy-tab]',wrap).forEach(b=>b.onclick=()=>go(b.dataset.academyTab));
