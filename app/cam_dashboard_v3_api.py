...
router = APIRouter(prefix="/api/cam/dashboard", tags=["cam-dashboard-v3"])
...
def _academy_payments(today: date) -> dict:
    ...
    academy = fetch_one(
        "SELECT COALESCE(SUM(amount_cents),0) AS total FROM academy_expenses WHERE expense_type='cam' AND status='paid' AND expense_date>=? AND expense_date<?",
        (start, end),
    ) or {"total": 0}
    return {
