...
router = APIRouter(prefix="/api/cam", tags=["cam-finance-operations"])
...
class ExpensePayload(BaseModel):
    expense_type: Literal["cam", "facility"]
...
def operations_summary_for_month(month: str | None = None) -> dict:
    ...
    academy_paid = fetch_one(
        ...
        WHERE expense_type='cam' AND status='paid' AND expense_date>=? AND expense_date<?
        """,
        (start, end),
    ) or {"total": 0, "count": 0}
...
def expenses(expense_type: str | None = None, month: str | None = None, status: str | None = None):
    ...
    if expense_type is not None:
        if expense_type not in {"cam", "facility"}:
