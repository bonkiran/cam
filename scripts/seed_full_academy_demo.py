    ...
    for attempt in range(1, 61):
        try:
            ...
            if storage and storage.get("database") == "postgresql":
                try:
                    summary = get("/api/academy/finance/operations-summary")
...
def ensure_players() -> list[dict]:
    current = get("/api/academy/players")
    ...
    for index, (first, last) in enumerate(PLAYER_NAMES):
        ...
        if existing:
            saved = put(f"/api/academy/players/{existing['id']}", payload)
            print("updated player", saved["name"], saved["id"])
        else:
            saved = post("/api/academy/players", payload)
...
def ensure_coaches() -> list[dict]:
    current = get("/api/academy/coaches")
    result = []
    for spec in COACHES:
        found = next(
            (
        ...
        if found:
            saved = put(f"/api/academy/coaches/{found['id']}", payload)
            print("updated coach", saved.get("preferred_name"), saved["id"])
        else:
            saved = post("/api/academy/coaches", payload)
...
def ensure_program(spec: dict) -> dict:
    ...
    if found:
        return put(f"/api/academy/programs/{found['id']}", payload)
    return post("/api/academy/programs", payload)
...
def ensure_family_account(player: dict, index: int) -> dict:
    ...
    return post("/api/academy/billing-accounts", payload)


def current_cycle_invoice_by_enrollment(month_start: date) -> dict[int, dict]:
    rows = get("/api/academy/invoices")
...
def ensure_coach_assignments(players: list[dict], coaches: list[dict]) -> None:
    existing = get("/api/academy/coach-player-assignments")
    active_pairs = {
...
def main():
    ...
    all_accounts = get("/api/academy/billing-accounts")
    ...
    for index, (player, enrollment) in enumerate(zip(players, enrollments)):
        ...
        invoice = get(f"/api/academy/invoices/{invoice['id']}")
    ...
    refreshed_players = get("/api/academy/players")
    ...
    target_coaches = [
        row for row in get("/api/academy/coaches")
        if (row.get("first_name"), row.get("last_name")) in target_coach_names
    ]
    rates = get("/api/academy/coach-rates?status=active")
    ...
    academy_expenses = get(f"/api/academy/expenses?expense_type=academy&month={cycle}")
    facility_expenses = get(f"/api/academy/expenses?expense_type=facility&month={cycle}")
