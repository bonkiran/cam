...
def test_parent_portal_login_add_card_full_payment_and_receipt():
    ...
    try:
        ...
        _json_request("PUT", "/api/cam/profile", {"name": "Parent Portal UI Academy"})
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                expect(page.locator("#camParentLoginForm")).to_be_visible()

                login = page.locator("#camParentLoginForm")
                ...
                expect(page.locator(".cam-parent-children").get_by_text("UI Aarav Patel", exact=True)).to_be_visible(timeout=15000)
                ...
                card_form = page.locator("#camSandboxCardForm")
                ...
                pay_form = page.locator("#camParentPayForm")
                ...
                receipt_panel = page.locator(".cam-parent-receipt")
                ...
                expect(receipt_panel.locator(".cam-parent-receipt-total").get_by_text("$175.00", exact=True)).to_be_visible()
            except Exception:
                Path("test-results").mkdir(exist_ok=True)
                page.screenshot(path="test-results/cam-parent-billing-ui-failure.png", full_page=True)
                raise
            finally:
                browser.close()
    finally:
