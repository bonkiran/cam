    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                page.locator('#academyPlayerForm [name="name"]').fill(player_name)
                page.get_by_role("button", name="Create Player").click()
                player_row = page.locator(".academy-player-row", has_text=player_name)
                expect(player_row).to_be_visible(timeout=10000)
                expect(player_row.locator(".academy-status")).to_have_text("active")
                ...
                expect(page.locator(".academy-player-row", has_text=player_name)).to_have_count(1)
                ...
                player_row = page.locator(".academy-player-row", has_text=player_name)
                expect(player_row).to_be_visible(timeout=10000)
                expect(player_row.locator(".academy-status")).to_have_text("inactive")

            except Exception:
                Path("test-results").mkdir(exist_ok=True)
                page.screenshot(path="test-results/academy-player-ui-failure.png", full_page=True)
                raise
            finally:
                browser.close()
    finally:
