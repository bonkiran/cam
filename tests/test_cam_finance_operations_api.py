...
def test_coach_rates_and_expense_summary_are_persistent_and_idempotent():
    client.put("/api/cam/profile", json={"name": "Finance Ops Test Academy"})
    coach = _post(
        "/api/cam/coaches",
        {
            "first_name": "Test",
    ...
    rate = _post("/api/cam/coach-rates", rate_payload)
    assert rate["coach_name"] == "Coach Test"
    assert int(rate["rate_cents"]) == 5500

    duplicate_rate = _post("/api/cam/coach-rates", rate_payload)
    ...
    assert academy_expense["expense_type"] == "cam"
    ...
    summary = client.get("/api/cam/finance/operations-summary?month=2026-08")
    ...
    rates = client.get("/api/cam/coach-rates?status=active").json()
    expenses = client.get("/api/cam/expenses?month=2026-08").json()
    assert len(rates) == 1
