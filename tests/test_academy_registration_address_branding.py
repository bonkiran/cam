...
from run import app
from app import academy_registration_address_validation as address_validation
...
def test_registration_branding_returns_academy_name_for_staff_and_parent():
    client.put("/api/academy/profile", json={"name": "JKC"})
    staff = client.get("/api/academy/registration/branding")
...
def test_registration_ui_uses_academy_name_in_message_and_form_title():
    ...
    assert "/api/academy/registration/branding" in staff_js
    assert "please complete the ${academyLabel()} player registration form" in staff_js
    assert "`${academyLabel()} Player Registration`" in staff_js

    assert "/api/public/registration/${encodeURIComponent(token)}/branding" in public_js
    assert "`${academyLabel(data?.academy_name)} Player Registration`" in public_js
