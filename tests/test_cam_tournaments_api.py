...
def test_tournament_team_registration_and_history():
    profile = client.put("/api/cam/profile", json={"name": "Tournament Test Academy"})
    assert profile.status_code == 200, profile.text

    team = _post("/api/cam/teams", {"name": "Tournament U15 XI", "age_group": "U15", "status": "active"})
    ...
    history = client.get(f"/api/cam/tournaments/{tournament_id}/entries").json()
    assert len(history) == 1
    assert history[0]["status"] == "withdrawn"
    tournament_final = client.get(f"/api/cam/tournaments/{tournament_id}").json()
    assert int(tournament_final["registered_team_count"]) == 0

    completed = client.put(
        f"/api/cam/tournaments/{tournament_id}",
        json={
            "name": "Southeast Junior Cup",
            "organizer": "Regional Cricket Association",
            "start_date": "2026-10-10",
