...
def _create_access(owner_token: str, **payload):
    return _post("/api/cam/access/users", payload, owner_token)
...
def test_player_reviews_rbac_publish_history_and_actions():
    profile = client.put("/api/cam/profile", json={"name": "Reviews Test Academy"})
    ...
    reference = client.get("/api/cam/reviews/reference", headers=_auth(coach_token))
    ...
    parent_draft_list = client.get("/api/cam/reviews", headers=_auth(parent_token))
    assert parent_draft_list.status_code == 200
    assert parent_draft_list.json() == []
    parent_draft_get = client.get(f"/api/cam/reviews/{review_id}", headers=_auth(parent_token))
    ...
    completed = client.put(
        f"/api/cam/reviews/{review_id}/actions/{action_id}",
    ...
    _post(f"/api/cam/reviews/{second['id']}/publish", {}, owner_token)

    trend = client.get(f"/api/cam/reviews/trend/{player['id']}", headers=_auth(parent_token))
    ...
    audit = client.get("/api/cam/access/audit?limit=100", headers=_auth(owner_token))
