...
def test_coach_profile_assignment_and_lifecycle():
    academy = client.put("/api/cam/profile", json={"name": "Coaches Test Academy"})
    assert academy.status_code == 200, academy.text

    player = client.post("/api/cam/players", json={"name": "Coach Assigned Player", "status": "active"})
    assert player.status_code == 201, player.text
    player_id = int(player.json()["id"])

    other_player = client.post("/api/cam/players", json={"name": "Coach Future Player", "status": "active"})
    ...
    detail = client.get(f"/api/cam/coaches/{coach_id}")
    ...
    history = client.get(f"/api/cam/coach-player-assignments?coach_id={coach_id}")
    ...
    ended = client.post(f"/api/cam/coach-player-assignments/{assignment_id}/end?end_date=2026-10-15", json={})
    assert ended.status_code == 200, ended.text
    assert ended.json()["status"] == "inactive"
    assert ended.json()["end_date"] == "2026-10-15"

    history_after = client.get(f"/api/cam/coach-player-assignments?coach_id={coach_id}")
