...
def test_batches_sessions_capacity_schedule_conflicts_and_workload():
    academy = client.put("/api/academy/profile", json={"name": "Batches Test Academy", "timezone": "America/New_York"})
    assert academy.status_code == 200, academy.text

    program = _post("/api/academy/programs", {"name": "U15 Batch Program", "program_type": "group", "status": "active"})
    coach1 = _post("/api/academy/coaches", {"first_name": "Batch", "last_name": "Coach", "specialties": ["Batting"], "status": "active"})
    coach2 = _post("/api/academy/coaches", {"first_name": "Private", "last_name": "Coach", "specialties": ["Bowling"], "status": "active"})
    p1 = _post("/api/academy/players", {"name": "Batch Player One", "status": "active"})
    p2 = _post("/api/academy/players", {"name": "Batch Player Two", "status": "active"})
    p3 = _post("/api/academy/players", {"name": "Batch Player Three", "status": "active"})
    ...
    batch = client.get(f"/api/academy/batches/{batch_id}").json()
    assert int(batch["primary_coach_id"]) == int(coach1["id"])

    m1 = _post(f"/api/academy/batches/{batch_id}/players", {"player_id": p1["id"], "joined_on": "2026-09-01"})
    m2 = _post(f"/api/academy/batches/{batch_id}/players", {"player_id": p2["id"], "joined_on": "2026-09-01"})
    assert m1["status"] == "active" and m2["status"] == "active"

    over_capacity = client.post(f"/api/academy/batches/{batch_id}/players", json={"player_id": p3["id"]})
    ...
    sessions = client.get(f"/api/academy/sessions?batch_id={batch_id}")
    ...
    updated = client.put(
        f"/api/academy/sessions/{first_id}",
        json={
    ...
    cancelled = client.post(f"/api/academy/sessions/{second_id}/cancel", json={"reason": "Facility closure"})
    assert cancelled.status_code == 200, cancelled.text
    assert cancelled.json()["status"] == "cancelled"
    assert cancelled.json()["cancellation_reason"] == "Facility closure"

    makeup = _post(
        f"/api/academy/sessions/{second_id}/makeup",
    ...
    makeup_players = client.get(f"/api/academy/sessions/{makeup['id']}/players").json()
    ...
    workload = client.get(f"/api/academy/coaches/{coach1['id']}/workload")
