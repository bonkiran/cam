...
def test_academy_access_roles_ui_end_to_end():
    ...
    try:
        ...
        _json_request("PUT", "/api/cam/profile", {"name": "Access UI Academy"})
        coach = _json_request(
            "POST",
            "/api/cam/coaches",
        ...
        with sync_playwright() as playwright:
            ...
            try:
                page.goto(f"{BASE_URL}/#cam?tab=access", wait_until="domcontentloaded")
                expect(page.get_by_role("heading", name="Access & Roles")).to_be_visible(timeout=15000)
                expect(page.locator("#camBootstrapForm")).to_be_visible()
                expect(page.get_by_text("Bootstrap is configured on the server.")).to_be_visible()

                bootstrap = page.locator("#camBootstrapForm")
                ...
                expect(page.locator(".cam-access-session").get_by_text("UI Academy Owner", exact=True)).to_be_visible()
                expect(page.locator("#camAddAccessUser")).to_be_visible()
                expect(page.locator(".cam-access-role-card")).to_have_count(5)
                expect(page.locator(".cam-access-user-row", has_text="ui-owner@example.test")).to_be_visible()

                # Create a coach account and link it to the actual coach record.
                page.locator("#camAddAccessUser").click()
                form = page.locator("#camAccessUserForm")
                ...
                coach_row = page.locator(".cam-access-user-row", has_text="coach-ui@example.test")
                ...
                page.locator("#camAddAccessUser").click()
                parent_form = page.locator("#camAccessUserForm")
                ...
                parent_row = page.locator(".cam-access-user-row", has_text="parent-ui@example.test")
                ...
                expect(page.locator(".cam-access-audit-row")).to_have_count(3)

                # Non-admin roles can authenticate and see only their own access profile.
                page.locator("#camAccessLogout").click()
                expect(page.locator("#camLoginForm")).to_be_visible(timeout=10000)
                login = page.locator("#camLoginForm")
                ...
                expect(page.locator("#camAddAccessUser")).to_have_count(0)
                expect(page.get_by_text("reviews · manage")).to_be_visible()
            except Exception:
                Path("test-results").mkdir(exist_ok=True)
                page.screenshot(path="test-results/cam-access-ui-failure.png", full_page=True)
                raise
            finally:
                browser.close()
    finally:
