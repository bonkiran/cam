...
def test_registration_invite_tracking_submission_review_and_approval():
    profile = client.put("/api/cam/profile", json={"name": "Registration Test Academy", "city": "Johns Creek", "state": "GA", "postal_code": "30024"})
    ...
    sent = client.post(f"/api/cam/registration/invites/{invite_id}/sent", json={"channel": "sms"})
    ...
    tracked = client.get("/api/cam/registration/invites").json()
    ...
    summary = client.get("/api/cam/registration/summary")
    assert summary.status_code == 200, summary.text
    assert int(summary.json()["submitted"]) == 1

    application = client.get(f"/api/cam/registration/applications/{application_id}")
    ...
    needs_info = client.post(
        f"/api/cam/registration/applications/{application_id}/review",
    ...
    player = client.get(f"/api/cam/players/{player_id}")
    ...
    final_summary = client.get("/api/cam/registration/summary").json()
