...
def test_fees_foundation_ui_end_to_end():
    ...
    try:
        ...
        _json_request("PUT", "/api/cam/profile", {"name": "Fees UI Academy"})
        ...
        program = _json_request("POST", "/api/cam/programs", {"name": "UI Fees U15", "program_type": "group", "status": "active"})
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                fee_form = page.locator("#camFeePlanForm")
                ...
                fee_row = page.locator(".cam-fee-row", has_text="UI U15 Monthly").first
                ...
                account_form = page.locator("#camBillingAccountForm")
                ...
                account_row = page.locator(".cam-fee-row", has_text="UI Patel Family").first
                ...
                billing_form = page.locator("#camEnrollmentBillingForm")
                ...
                fee_plan_id = page.evaluate("""
                    async () => (await (await fetch('/api/cam/fee-plans')).json()).find(x => x.name === 'UI U15 Monthly').id
                """)
                ...
                configured_row = page.locator(".cam-fee-row", has_text="UI Fees Aarav · UI Fees U15")
                ...
                account_id = page.evaluate("""
                    async () => (await (await fetch('/api/cam/billing-accounts')).json()).find(x => x.account_name === 'UI Patel Family').id
                """)
                for issue_date, due_date in (("2026-09-01", "2026-09-15"), ("2026-10-01", "2026-10-15")):
                    page.locator("#openInvoiceForm").click()
                    invoice_form = page.locator("#camEnrollmentInvoiceForm")
                    ...
                    expect(page.locator("#camEnrollmentInvoiceForm")).to_have_count(0, timeout=10000)

                invoice_rows = page.locator(".cam-invoice-row", has_text="UI Patel Family")
                ...
                invoices = page.evaluate(
                    "async (id) => await (await fetch(`/api/cam/invoices?account_id=${id}`)).json()",
                    account_id,
                )
                ...
                account = page.evaluate("async (id) => await (await fetch(`/api/cam/billing-accounts/${id}`)).json()", account_id)
                assert int(account["balance_cents"]) == 36000
            except Exception:
                Path("test-results").mkdir(exist_ok=True)
                page.screenshot(path="test-results/cam-fees-ui-failure.png", full_page=True)
                raise
            finally:
                browser.close()
    finally:
