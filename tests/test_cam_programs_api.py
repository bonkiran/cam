...
def _create_player(name: str, status: str = "active") -> int:
    response = client.post("/api/cam/players", json={"name": name, "status": status})
...
def test_programs_and_enrollment_lifecycle_end_to_end():
    ...
    freeze = client.post(
        f"/api/cam/enrollments/{regular_id}/freeze",
    ...
    cancel = client.post(
        f"/api/cam/enrollments/{regular_id}/cancel",
    ...
    history = client.get(f"/api/cam/enrollments?player_id={player_id}")
    assert history.status_code == 200
    history_rows = history.json()
    assert len(history_rows) == 2
    assert {row["status"] for row in history_rows} == {"cancelled", "active"}

    program_detail = client.get(f"/api/cam/programs/{program_id}")
    ...
    trial_cancel = client.post(
        f"/api/cam/enrollments/{trial_id}/cancel",
