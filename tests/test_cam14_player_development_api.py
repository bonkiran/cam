...
def _setup_session(tag: str):
    _put("/api/cam/profile", {"name": f"CAM-14 Test Academy {tag}", "timezone": "America/New_York"})
    program = _post("/api/cam/programs", {"name": f"U13 Development {tag}", "program_type": "group", "status": "active"})
    coach = _post("/api/cam/coaches", {"first_name": "Moat", "last_name": f"Coach {tag}", "status": "active"})
    players = [
        _post("/api/cam/players", {"name": f"CAM14 {tag} Player {index}", "status": "active"})
        for index in (1, 2, 3)
    ]
    batch = _post(
        "/api/cam/batches",
    ...
    _post(
        f"/api/cam/batch-coach-assignments?batch_id={batch['id']}",
...
def test_cam14_passive_practice_evidence_tracks_focus_and_attendance_without_claiming_improvement():
    ...
    skills = client.get("/api/cam/development/skills")
    ...
    focus = _put(
        f"/api/cam/sessions/{session_id}/development-focus",
    ...
    for attended_player in (p1, p2):
        history_response = client.get(f"/api/cam/players/{attended_player['id']}/development-history")
    ...
    absent_history = client.get(f"/api/cam/players/{p3['id']}/development-history")
    ...
    assert client.get(f"/api/cam/players/{p2['id']}/development-history").json()["evidence"] == []
    ...
    p1_history = client.get(f"/api/cam/players/{p1['id']}/development-history").json()["evidence"]
    assert len(p1_history) == 1
    assert p1_history[0]["skill_key"] == "cover_drive"

    summary = client.get(f"/api/cam/players/{p1['id']}/development-summary")
    ...
    assert len(client.get(f"/api/cam/players/{p1['id']}/development-history").json()["evidence"]) == 1
