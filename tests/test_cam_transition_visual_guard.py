...
def test_transition_snapshot_never_paints_previous_page_panels():
    ...
    assert "#camTransitionSnapshot > .cam-content" in css
    assert "display:none !important" in css
    assert "#camTransitionSnapshot{" in css
    ...
    assert "#camTransitionSnapshot .cam-transition-progress" in css
    assert "camTransitionSnapshot::before" in css
