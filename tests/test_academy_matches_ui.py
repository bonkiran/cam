...
def test_teams_matches_ui_end_to_end():
    ...
    try:
        ...
        _json_request("PUT", "/api/academy/profile", {"name": "Matches UI Academy"})
        p1 = _json_request("POST", "/api/academy/players", {"name": "UI Match Aarav", "status": "active"})
        p2 = _json_request("POST", "/api/academy/players", {"name": "UI Match Maya", "status": "active"})
        p3 = _json_request("POST", "/api/academy/players", {"name": "UI Match Rohan", "status": "active"})

        with sync_playwright() as playwright:
            browser = playwright.chromium.launch(headless=True)
            page = browser.new_page(viewport={"width": 1600, "height": 1100})
            try:
                page.goto(f"{BASE_URL}/#academy?tab=teams", wait_until="domcontentloaded")
                ...
                form = page.locator("#academyTeamForm")
                ...
                team_row = page.locator(".academy-match-team-row", has_text="UI U15 Match XI")
                ...
                for player in (p1, p2, p3):
                    ...
                    roster_form = page.locator("#academyTeamRosterForm")
                    ...
                    expect(page.locator("#academyTeamRosterForm")).to_have_count(0, timeout=10000)
                ...
                match_row = page.locator(".academy-match-row", has_text="UI North Atlanta Juniors")
                ...
                squad_form = page.locator("#academySquadForm")
                ...
                result_form = page.locator("#academyMatchResultForm")
                ...
                persisted = page.evaluate("""
                    async (id) => await (await fetch(`/api/academy/matches/${id}/stats`)).json()
                """, match_id)
                ...
                page.screenshot(path="test-results/academy-matches-ui-failure.png", full_page=True)
                raise
            finally:
                browser.close()
    finally:
