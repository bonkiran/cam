...
def _watch_transition(page, target_hash: str) -> dict:
    return page.evaluate(
        ...
        async (targetHash) => {
          ...
          const hadWorkspace = !!document.querySelector('#camWorkspace');
          const isVisible = (el) => {
            if (!el) return false;
          ...
          for (let i = 0; i < 180; i++) {
            await frame();
            result.frames += 1;
            result.guardVersion = document.documentElement.dataset.camRouteGuard || null;
            result.adapterVersion = document.documentElement.dataset.camRouterAdapter || null;
            const pending = document.documentElement.classList.contains('cam-route-pending');
            result.sawLoadingState = result.sawLoadingState || pending;

            if (hadWorkspace && !document.querySelector('#camWorkspace')) {
            ...
            const legacyHero = document.querySelector('#camWorkspace .cam-content > .cam-hero');
            ...
            const workspace = document.querySelector('#camWorkspace .cam-content');
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                expect(page.locator("#camWorkspace")).to_be_visible(timeout=15000)
                expect(page.locator("#camWorkspace .cam-content")).to_be_visible(timeout=15000)
                assert page.evaluate("document.documentElement.dataset.camRouteGuard") == "3"
                assert page.evaluate("document.documentElement.dataset.camRouterAdapter") == "1"

                # The legacy dashboard is retired from the visible Academy overview.
                expect(page.locator("#camWorkspace .cam-content > .cam-hero")).to_have_count(0)
                ...
                players = _watch_transition(page, "cam?tab=players")
                ...
                back_overview = _watch_transition(page, "cam")
                expect(page.locator("#camWorkspace")).to_be_visible(timeout=15000)
