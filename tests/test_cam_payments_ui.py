...
def _setup(tag: str):
    _json_request("PUT", "/api/cam/profile", {"name": f"Payment UI Academy {tag}"})
    p1 = _json_request("POST", "/api/cam/players", {"name": f"UI Pay Aarav {tag}", "status": "active"})
    p2 = _json_request("POST", "/api/cam/players", {"name": f"UI Pay Maya {tag}", "status": "active"})
    program = _json_request("POST", "/api/cam/programs", {"name": f"UI Pay U15 {tag}", "program_type": "group", "status": "active"})
    e1 = _json_request("POST", "/api/cam/enrollments", {"player_id": p1["id"], "program_id": program["id"], "enrollment_type": "regular", "start_date": "2026-09-01"})
    e2 = _json_request("POST", "/api/cam/enrollments", {"player_id": p2["id"], "program_id": program["id"], "enrollment_type": "regular", "start_date": "2026-09-01"})
    plan = _json_request("POST", "/api/cam/fee-plans", {"name": f"UI Pay Monthly 200 {tag}", "amount_cents": 20000, "billing_frequency": "monthly", "due_day_of_month": 1, "program_id": program["id"], "status": "active"})
    for enrollment in (e1, e2):
        _json_request("PUT", f"/api/cam/enrollments/{enrollment['id']}/billing", {"fee_plan_id": plan["id"], "discount_type": "none", "discount_value": 0})
    account = _json_request("POST", "/api/cam/billing-accounts", {"account_name": f"UI Pay Family {tag}", "player_ids": [p1["id"], p2["id"]], "overpayment_allowed": True, "status": "active"})
    i1 = _json_request("POST", "/api/cam/invoices/from-enrollment", {"account_id": account["id"], "enrollment_id": e1["id"], "issue_date": "2026-09-01", "due_date": "2026-09-15"})
    i2 = _json_request("POST", "/api/cam/invoices/from-enrollment", {"account_id": account["id"], "enrollment_id": e2["id"], "issue_date": "2026-09-01", "due_date": "2026-09-15"})
...
def _post_payment(page, account_id, invoice_id, amount, method, reference=""):
    ...
    form = page.locator("#camPaymentForm")
    ...
    expect(page.locator("#camPaymentForm")).to_have_count(0, timeout=10000)
...
def test_payment_ledger_ui_partial_full_credit_receipt_refund_and_reconciliation():
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                page.goto(f"{BASE_URL}/#cam?tab=fees", wait_until="domcontentloaded")
                ...
                partial_row = page.locator(".cam-payment-row", has_text=cash_ref)
                ...
                invoice1 = page.evaluate("async (id) => await (await fetch(`/api/cam/invoices/${id}`)).json()", i1["id"])
                ...
                check_row = page.locator(".cam-payment-row", has_text=check_ref)
                expect(check_row).to_be_visible(timeout=10000)
                check_row.get_by_role("button", name="Receipt").click()
                receipt = page.locator(".cam-payment-receipt")
                ...
                overpay_row = page.locator(".cam-payment-row", has_text=card_ref)
                expect(overpay_row).to_be_visible(timeout=10000)
                expect(overpay_row).to_contain_text("Credit $50.00")
                ledger_row = page.locator(".cam-ledger-row", has_text=account_name)
                ...
                refund_form = page.locator("#camRefundForm")
                ...
                expect(page.locator("#camRefundForm")).to_have_count(0, timeout=10000)
                overpay_row = page.locator(".cam-payment-row", has_text=card_ref)
                ...
                invoice2 = page.evaluate("async (id) => await (await fetch(`/api/cam/invoices/${id}`)).json()", i2["id"])
                ...
                ledger = page.evaluate("async (id) => await (await fetch(`/api/cam/billing-accounts/${id}/ledger`)).json()", account_id)
                ...
                page.screenshot(path="test-results/cam-payments-ui-failure.png", full_page=True)
                raise
            finally:
                browser.close()
    finally:
