...
def seed():
    ...
    player=request("POST","/api/cam/players",{
    ...
    program=request("POST","/api/cam/programs",{"name":"Manual UX Program","program_type":"group","status":"active"},owner)
    enrollment=request("POST","/api/cam/enrollments",{
        "player_id":player["id"],"program_id":program["id"],"enrollment_type":"regular","start_date":date.today().isoformat()
    },owner)
    batch=request("POST","/api/cam/batches",{
        "name":"Manual UX Batch","program_id":program["id"],"capacity":12,"status":"active"
    },owner)
    request("POST",f"/api/cam/batches/{batch['id']}/players",{
        "player_id":player["id"],"waitlist_if_full":False,"joined_on":date.today().isoformat()
    },owner)
    fee=request("POST","/api/cam/fee-plans",{
    ...
    request("POST","/api/cam/access/users",{
    ...
    request("POST","/api/cam/parent/payment-methods/sandbox",{
...
def test_dashboard_setup_batch_invoice_and_parent_payment_ui():
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                page.add_init_script(f"sessionStorage.setItem('cam-cam-session-v1', {json.dumps(owner_token)});")
                ...
                timezone=page.locator('#camProfileForm [name="timezone"]')
                ...
                row=page.locator('.cam-batch-membership-row',has_text='Manual UX Player')
                expect(row).to_be_visible(timeout=15000)
                expect(row.get_by_role("button",name="Remove")).to_be_visible(timeout=10000)
                status_box=row.locator(':scope > .cam-program-status').bounding_box()
                actions_box=row.locator(':scope > .cam-roster-lifecycle-actions').bounding_box()
                ...
                invoice_row=page.locator('.cam-invoice-row',has_text=invoice['invoice_number'])
                ...
                open_button.click()
                expect(page.locator('.cam-invoice-detail').get_by_role("heading",name=invoice['invoice_number'])).to_be_visible(timeout=10000)
                expect(page.locator('.cam-invoice-detail')).to_contain_text("Balance")

                page.evaluate("(token) => sessionStorage.setItem('cam-cam-session-v1', token)",parent_token)
                page.goto(f"{BASE_URL}/#cam?tab=parent",wait_until="domcontentloaded")
                ...
                pay.click()
                amount=page.locator('#camParentPayForm [name="amount"]')
