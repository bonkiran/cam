...
def _save(page):
    ...
    with page.expect_response(
        lambda response: f"/api/cam/sessions/{session_id}/attendance" in response.url
        and response.request.method == "PUT",
        timeout=10000,
    ) as response_info:
        page.get_by_role("button", name="Save Attendance").click()
    assert response_info.value.ok
    expect(page.locator("#camAttendanceForm")).to_have_attribute(
...
def test_attendance_ui_end_to_end():
    ...
    try:
        ...
        _json_request("PUT", "/api/cam/profile", {"name": "Attendance UI Academy", "timezone": "America/New_York"})
        program = _json_request("POST", "/api/cam/programs", {"name": "Attendance UI Program", "program_type": "group", "status": "active"})
        coach = _json_request("POST", "/api/cam/coaches", {"first_name": "UI", "last_name": "Attendance Coach", "status": "active"})
        p1 = _json_request("POST", "/api/cam/players", {"name": "UI Attendance Player One", "status": "active"})
        p2 = _json_request("POST", "/api/cam/players", {"name": "UI Attendance Player Two", "status": "active"})
        batch = _json_request("POST", "/api/cam/batches", {"name": "Attendance UI Batch", "program_id": program["id"], "capacity": 5, "status": "active"})
        _json_request("POST", f"/api/cam/batch-coach-assignments?batch_id={batch['id']}", {"coach_id": coach["id"], "assignment_role": "primary", "start_date": "2026-09-01"})
        _json_request("POST", f"/api/cam/batches/{batch['id']}/players", {"player_id": p1["id"], "joined_on": "2026-09-01"})
        _json_request("POST", f"/api/cam/batches/{batch['id']}/players", {"player_id": p2["id"], "joined_on": "2026-09-01"})
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                _save(page)
                expect(page.locator(".cam-attendance-history")).to_contain_text("present → late", timeout=10000)
                ...
                row2.locator('[name="absence_reason"]').fill("Sick")
                _save(page)
                alert = page.locator(".cam-attendance-alert", has_text=p2["name"])
                ...
                _save(page)
                expect(page.locator(".cam-attendance-alert", has_text=p2["name"])).to_have_count(0, timeout=10000)
                ...
                with page.expect_response(
                    lambda response: response.url.endswith("/api/cam/attendance/policy")
                    and response.request.method == "PUT",
                    timeout=10000,
                ) as response_info:
                    page.get_by_role("button", name="Save Policy").click()
                assert response_info.value.ok
                expect(page.locator(".cam-stat", has_text="Alert threshold").locator("strong")).to_have_text("4", timeout=10000)

            except Exception:
                Path("test-results").mkdir(exist_ok=True)
                page.screenshot(path="test-results/cam-attendance-ui-failure.png", full_page=True)
                raise
            finally:
                browser.close()
    finally:
