...
def test_fee_plan_family_account_enrollment_discount_and_unique_invoices():
    profile = client.put("/api/cam/profile", json={"name": "Fees Test Academy"})
    ...
    other_player = _post("/api/cam/players", {"name": "Fees Other Player", "status": "active"})
    program = _post(
        "/api/cam/programs",
    ...
    numbers = [row["invoice_number"] for row in client.get("/api/cam/invoices").json()]
    assert len(numbers) == len(set(numbers))
    assert {invoice1["invoice_number"], invoice2["invoice_number"]}.issubset(set(numbers))

    account_after = client.get(f"/api/cam/billing-accounts/{account_id}")
