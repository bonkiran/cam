    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                page.goto(f"{BASE_URL}/#cam?tab=setup", wait_until="domcontentloaded")
                expect(page.get_by_role("heading", name="Academy Setup")).to_be_visible(timeout=15000)
                form = page.locator("#camProfileForm")
                ...
                pform = page.locator("#camPlayerForm")
                ...
                prow = page.locator(".cam-player-row", has_text="Extended Test Player")
                expect(prow).to_be_visible(timeout=10000)
                expect(prow.locator(".cam-status")).to_have_text("waitlisted")
                ...
                page.get_by_role("button", name="Create Player").click()
                expect(page.locator(".cam-player-row", has_text=special_name)).to_be_visible(timeout=10000)
                ...
                expect(page.get_by_role("heading", name="Academy Players")).to_be_visible(timeout=15000)
                legacy_row = page.locator(".cam-player-row", has_text=legacy_name)
                ...
                page.screenshot(path="test-results/cam-extended-ui-failure.png", full_page=True)
                raise
            finally:
                browser.close()
    finally:
