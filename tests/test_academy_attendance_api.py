...
def test_attendance_statuses_corrections_metrics_makeup_coach_and_alerts():
    _put("/api/academy/profile", {"name": "Attendance Test Academy", "timezone": "America/New_York"})

    policy = client.get("/api/academy/attendance/policy")
    assert policy.status_code == 200, policy.text
    assert policy.json()["repeated_absence_threshold"] == 3
    assert policy.json()["absence_lookback_days"] == 30

    program = _post("/api/academy/programs", {"name": "Attendance Program", "program_type": "group", "status": "active"})
    coach = _post("/api/academy/coaches", {"first_name": "Attendance", "last_name": "Coach", "status": "active"})
    player1 = _post("/api/academy/players", {"name": "Attendance Player One", "status": "active"})
    player2 = _post("/api/academy/players", {"name": "Attendance Player Two", "status": "active"})
    batch = _post(
        "/api/academy/batches",
    ...
    _post(f"/api/academy/batches/{batch_id}/players", {"player_id": player1["id"], "joined_on": "2026-09-01"})
    _post(f"/api/academy/batches/{batch_id}/players", {"player_id": player2["id"], "joined_on": "2026-09-01"})
    ...
    first = _put(
        f"/api/academy/sessions/{session_ids[0]}/attendance",
        {
            "players": [
    ...
    history = client.get(
        f"/api/academy/attendance/history?session_id={session_ids[0]}&entity_type=player&subject_id={player1['id']}"
    )
    ...
    _put(
        f"/api/academy/sessions/{session_ids[1]}/attendance",
        {
            "players": [
                {"player_id": player1["id"], "status": "present"},
    ...
    _put(
        f"/api/academy/sessions/{session_ids[2]}/attendance",
        {
            "players": [
    ...
    alerts = client.get(f"/api/academy/attendance/alerts?player_id={player2['id']}")
    ...
    assert len(client.get(f"/api/academy/attendance/alerts?player_id={player2['id']}").json()) == 1

    _put(
        f"/api/academy/sessions/{session_ids[3]}/attendance",
        {
            "players": [
    ...
    summary = client.get(f"/api/academy/players/{player1['id']}/attendance-summary")
    ...
    assert client.get(f"/api/academy/attendance/alerts?player_id={player2['id']}").json() == []
    resolved = client.get(f"/api/academy/attendance/alerts?status=resolved&player_id={player2['id']}")
