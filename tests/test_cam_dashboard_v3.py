...
import app.cam_dashboard_v3_api as dashboard_v3
...
def test_dashboard_v4_matches_approved_c17_prototype_contract():
    ...
    assert "/api/cam/dashboard/v3" in js
    assert "/api/cam/enrollments" in js
    ...
    assert "body.c17-cam-mode #camWorkspace .cam-tabs{display:none!important}" in shell_css
    ...
    assert "{label:'Dashboard', icon:'⌂', target:'cam'" in shell_js
    assert "active:r => r.page==='cam' && r.tab==='overview'" in shell_js
...
def test_dashboard_v3_api_attendance_does_not_expose_excused_column():
    source = (REPO_ROOT / "app" / "cam_dashboard_v3_api.py").read_text(encoding="utf-8")
