...
def test_programs_and_enrollment_ui_end_to_end():
    ...
    try:
        ...
        _json_request("PUT", "/api/academy/profile", {"name": "Programs UI Academy"})
        regular_player = _json_request(
            "POST", "/api/academy/players", {"name": "Programs UI Regular Player", "status": "active"}
        )
        trial_player = _json_request(
            "POST", "/api/academy/players", {"name": "Programs UI Trial Player", "status": "active"}
        )
        ...
        with sync_playwright() as playwright:
            ...
            try:
                page.goto(f"{BASE_URL}/#academy?tab=programs", wait_until="domcontentloaded")
                ...
                program_form = page.locator("#academyProgramForm")
                ...
                program_row = page.locator(".academy-program-row", has_text="UI U13 Development")
                ...
                program_row = page.locator(".academy-program-row", has_text="UI U13 Development Plus")
                expect(program_row).to_be_visible(timeout=10000)

                # Regular enrollment.
                page.get_by_role("button", name="Enroll Player").click()
                enrollment_form = page.locator("#academyEnrollmentForm")
                ...
                regular_row = page.locator(".academy-enrollment-row", has_text=regular_player["name"])
                ...
                page.locator("#academyEnrollmentForm").get_by_role("button", name="Cancel").click()

                # Freeze the regular enrollment and retain it in history.
                regular_row = page.locator(".academy-enrollment-row", has_text=regular_player["name"])
                regular_row.get_by_role("button", name="Freeze").click()
                action_form = page.locator("#academyEnrollmentActionForm")
                ...
                page.get_by_role("button", name="Create Enrollment").click()
                trial_row = page.locator(".academy-enrollment-row", has_text=trial_player["name"])
                ...
                expect(page.locator(".academy-enrollment-row", has_text=regular_player["name"])).to_have_count(1)
                expect(page.locator(".academy-enrollment-row", has_text=trial_player["name"])).to_have_count(1)
                trial_row = page.locator(".academy-enrollment-row", has_text=trial_player["name"])
                expect(trial_row).to_contain_text("active")

            except Exception:
                Path("test-results").mkdir(exist_ok=True)
                page.screenshot(path="test-results/academy-programs-ui-failure.png", full_page=True)
                raise
            finally:
                browser.close()
    finally:
