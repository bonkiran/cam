...
def test_academy_roles_security_end_to_end():
    ...
    no_auth_users = client.get("/api/cam/access/users")
    ...
    reference = client.get("/api/cam/access/reference", headers=_auth(owner_token))
    ...
    roles = client.get("/api/cam/access/roles", headers=_auth(owner_token))
    ...
    users = client.get("/api/cam/access/users", headers=_auth(owner_token))
    ...
    forbidden = client.get("/api/cam/access/users", headers=_auth(coach_token))
    assert forbidden.status_code == 403

    admin_login = _post("/api/auth/login", {"email": "admin@example.test", "password": "AdminPass!123"})
    admin_token = admin_login["token"]
    admin_can_list = client.get("/api/cam/access/users", headers=_auth(admin_token))
    assert admin_can_list.status_code == 200

    disable_coach = client.put(
        f"/api/cam/access/users/{coach_user['id']}",
        json={
            "display_name": "Coach Anita",
            "email": "coach@example.test",
            "role": "coach",
    ...
    reset = client.post(
        f"/api/cam/access/users/{coach_user['id']}/password",
