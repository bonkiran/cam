...
def test_teams_fixtures_squad_result_and_player_statistics():
    academy = client.put("/api/academy/profile", json={"name": "Matches Test Academy", "timezone": "America/New_York"})
    assert academy.status_code == 200, academy.text

    p1 = _post("/api/academy/players", {"name": "Match Player One", "status": "active"})
    p2 = _post("/api/academy/players", {"name": "Match Player Two", "status": "active"})
    p3 = _post("/api/academy/players", {"name": "Match Player Three", "status": "active"})
    outsider = _post("/api/academy/players", {"name": "Match Outsider", "status": "active"})
    ...
    for index, player in enumerate((p1, p2, p3), start=1):
        roster = _post(
            f"/api/academy/teams/{team_id}/roster",
    ...
    team_after = client.get(f"/api/academy/teams/{team_id}")
    ...
    outsider_squad = client.put(
        f"/api/academy/matches/{match_id}/squad",
    ...
    result = client.put(
        f"/api/academy/matches/{match_id}/result",
        json={
            "outcome": "win",
            "our_score": "146/5",
    ...
    completed = client.get(f"/api/academy/matches/{match_id}")
