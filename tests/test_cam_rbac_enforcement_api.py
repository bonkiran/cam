...
def test_legacy_management_is_owner_admin_only_after_bootstrap():
    ...
    profile = client.put("/api/cam/profile", json={"name": "RBAC Test Academy"})
    ...
    anonymous_players = client.get("/api/cam/players")
    ...
    owner_players = client.get("/api/cam/players", headers=_auth(owner_token))
    ...
    admin_programs = client.get("/api/cam/programs", headers=_auth(admin_token))
    ...
    for role_token in (coach_token, parent_token, player_token):
        denied_players = client.get("/api/cam/players", headers=_auth(role_token))
        assert denied_players.status_code == 403, denied_players.text
        denied_fees = client.get("/api/cam/fee-plans", headers=_auth(role_token))
    ...
    parent_reviews = client.get("/api/cam/reviews", headers=_auth(parent_token))
    assert parent_reviews.status_code == 200, parent_reviews.text
    assert parent_reviews.json() == []

    access_roles = client.get("/api/cam/access/roles", headers=_auth(owner_token))
    assert access_roles.status_code == 200, access_roles.text
    parent_access_roles = client.get("/api/cam/access/roles", headers=_auth(parent_token))
