...
def _set_session(page, token: str) -> None:
    ...
    page.goto(f"{BASE_URL}/#cam?tab=reviews", wait_until="domcontentloaded")
...
def test_player_reviews_staff_to_parent_publish_flow():
    ...
    try:
        ...
        _json_request("PUT", "/api/cam/profile", {"name": "Reviews UI Academy"})
        coach = _json_request(
            "POST",
            "/api/cam/coaches",
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                expect(staff.locator("#camNewReview")).to_be_visible()

                staff.locator("#camNewReview").click()
                form = staff.locator("#camReviewForm")
                ...
                card = staff.locator(".cam-review-card", has_text="August Development Review")
                ...
                expect(parent.locator("#camNewReview")).to_have_count(0)

                card.get_by_role("button", name="Publish report card").click()
                published_card = staff.locator(".cam-review-card", has_text="August Development Review")
                expect(published_card).to_contain_text("published", timeout=10000)

                parent.reload(wait_until="domcontentloaded")
                parent_card = parent.locator(".cam-review-card", has_text="August Development Review")
                ...
                action_button.click()
                updated_card = staff.locator(".cam-review-card", has_text="August Development Review")
                expect(updated_card.locator(".cam-review-action.completed")).to_have_count(1, timeout=10000)
                expect(updated_card).to_contain_text("0 open")
                expect(updated_card.get_by_role("button", name="Reopen")).to_be_visible()
            except Exception:
                Path("test-results").mkdir(exist_ok=True)
                staff.screenshot(path="test-results/cam-reviews-staff-failure.png", full_page=True)
                parent.screenshot(path="test-results/cam-reviews-parent-failure.png", full_page=True)
                raise
            finally:
                browser.close()
    finally:
