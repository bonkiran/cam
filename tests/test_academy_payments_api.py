...
def _setup_family(prefix: str, *, overpayment_allowed: bool = True):
    client.put("/api/academy/profile", json={"name": f"{prefix} Academy"})
    p1 = _post("/api/academy/players", {"name": f"{prefix} Aarav", "status": "active"})
    p2 = _post("/api/academy/players", {"name": f"{prefix} Maya", "status": "active"})
    program = _post("/api/academy/programs", {"name": f"{prefix} U15", "program_type": "group", "status": "active"})
    e1 = _post("/api/academy/enrollments", {"player_id": p1["id"], "program_id": program["id"], "enrollment_type": "regular", "start_date": "2026-09-01"})
    e2 = _post("/api/academy/enrollments", {"player_id": p2["id"], "program_id": program["id"], "enrollment_type": "regular", "start_date": "2026-09-01"})
    plan = _post(
    ...
    for enrollment in (e1, e2):
        response = client.put(f"/api/academy/enrollments/{enrollment['id']}/billing", json={"fee_plan_id": plan["id"], "discount_type": "none", "discount_value": 0})
    ...
    i1 = _post("/api/academy/invoices/from-enrollment", {"account_id": account["id"], "enrollment_id": e1["id"], "issue_date": "2026-09-01", "due_date": "2026-09-15"})
    i2 = _post("/api/academy/invoices/from-enrollment", {"account_id": account["id"], "enrollment_id": e2["id"], "issue_date": "2026-09-01", "due_date": "2026-09-15"})
...
def test_payment_lifecycle_full_partial_credit_refund_receipt_idempotency_and_family_reconciliation():
    ...
    invoice1_partial = client.get(f"/api/academy/invoices/{i1['id']}").json()
    ...
    invoice1_paid = client.get(f"/api/academy/invoices/{i1['id']}").json()
    ...
    receipt = client.get(f"/api/academy/payments/{final['id']}/receipt")
    ...
    family_payments = client.get(f"/api/academy/payments?account_id={account_id}").json()
    ...
    invoice2_paid = client.get(f"/api/academy/invoices/{i2['id']}").json()
    assert invoice2_paid["status"] == "paid"
    assert int(invoice2_paid["balance_due_cents"]) == 0
    ledger_credit = client.get(f"/api/academy/billing-accounts/{account_id}/ledger").json()
    ...
    refunded = _post(
        f"/api/academy/payments/{overpay['id']}/refunds",
        {
            "amount_cents": 7000,
    ...
    invoice2_reopened = client.get(f"/api/academy/invoices/{i2['id']}").json()
    ...
    ledger = client.get(f"/api/academy/billing-accounts/{account_id}/ledger")
...
def test_excess_payment_is_rejected_when_family_disallows_overpayment():
    ...
    unchanged = client.get(f"/api/academy/invoices/{invoice['id']}").json()
    assert unchanged["status"] == "open"
    assert int(unchanged["amount_paid_cents"]) == 0
    assert client.get(f"/api/academy/payments?account_id={account['id']}").json() == []
