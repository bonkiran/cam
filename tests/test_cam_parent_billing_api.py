...
def test_parent_login_saved_test_card_full_payment_receipt_and_family_isolation():
    ...
    profile = client.put("/api/cam/profile", json={"name": "Parent Billing Test Academy"})
    ...
    no_auth = client.get("/api/cam/parent/billing")
    assert no_auth.status_code == 401

    summary = client.get("/api/cam/parent/billing", headers=_auth(parent_token))
    ...
    failed = client.post(
        f"/api/cam/parent/invoices/{family['invoice']['id']}/pay",
        json={"payment_method_id": declined_method["id"], "amount_cents": 17500},
        headers=_auth(parent_token),
    )
    assert failed.status_code == 402
    after_failed = client.get("/api/cam/parent/billing", headers=_auth(parent_token)).json()
    ...
    after_partial = client.get("/api/cam/parent/billing", headers=_auth(parent_token)).json()
    ...
    receipt = client.get(f"/api/cam/parent/receipts/{payment_id}", headers=_auth(parent_token))
    ...
    cross_family = client.post(
        f"/api/cam/parent/invoices/{other_family['invoice']['id']}/pay",
    ...
    removed = client.delete(
        f"/api/cam/parent/payment-methods/{declined_method['id']}",
        headers=_auth(parent_token),
    )
    assert removed.status_code == 204
    methods_after = client.get("/api/cam/parent/payment-methods", headers=_auth(parent_token)).json()
    assert [row["last4"] for row in methods_after["payment_methods"]] == ["4242"]

    audit = client.get("/api/cam/parent/billing-audit", headers=_auth(parent_token))
